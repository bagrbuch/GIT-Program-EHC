...
def start_LaTeX(output_data):
    ...
    with open(LATEX_FILE, "w") as latex_file:
        ...
        latex_file.write("This file is a summary of the measurement results and everything contained is generated by a Python program. \\ \n")
        latex_file.write("\\\ \n")
...
def export_LaTeX_1(output_data, LATEX_FILE):
    ...
    with open(LATEX_FILE, "a") as latex_file:
        ...
        latex_file.write("Voltage & \multirow{2}{*}{Meas.} & Duration of meas. & Max. Pressure & Consumption & Pressure Time & Equilibrium Time \\\ (V) &  & (h) & (bar) & (kWh/kg) & (h) & (h) \\\\\n")
        latex_file.write("\\hline \n")
        ...
        latex_file.write("This table presents measurements for different voltages. For each measurement, it includes the duration and the maximum pressure reached. The consumption value at this maximum pressure and the time at which it occurred are also recorded. Furthermore, the table indicates the time when the measurement reached equilibrium, defined as when the pressure was 99\% of the maximum value.\\ \n")
        latex_file.write("\\vspace{10mm}\n")

        # Second table
        latex_file.write("\\section*{Table 3: Efficiency and related data}\n")
        latex_file.write("\\vspace{5mm}\n")
        latex_file.write("\\setlength{\LTleft}{-3.25 cm} \n")
        latex_file.write("\\begin{longtable}{|c|c|c|c||c|c|c|c|c|} \n")
        latex_file.write("\\hline \n")
        latex_file.write("Voltage & \multirow{2}{*}{Meas.} & Max. eff. & Eff. time & Pressure & Rel. pressure & Consumption & Mass & Current density \\\ (V) &  & \% & (h) & (bar) & \% & (kWh/kg) & (mg/(d·$cm^2$)) & (mA/$cm^2$) \\\\\n")
        latex_file.write("\\hline \n")
        latex_file.write("\\endfirsthead \n")
        latex_file.write("\\hline \n")
        latex_file.write("Voltage & \multirow{2}{*}{Meas.} & Max. eff. & Eff. time & Pressure & Rel. pressure & Consumption & Mass & Current density \\\ (V) &  & \% & (h) & (bar) & \% & (kWh/kg) & (mg/(d·$cm^2$)) & (mA/$cm^2$) \\\\\n")
        ...
        latex_file.write("\\hline \n")
        latex_file.write("\\end{longtable}\n")
        latex_file.write("This table is based on the maximum overall efficiency value for each measurement. The time when this efficiency occurred was identified and for this time, the following data are listed: pressure and its percentage in relation to the maximum pressure of the measurement (Rel. Pressure), consumption, mass generated for one day, and current density. \\ \n")
        latex_file.write("\\newpage \n")
...
def export_LaTeX_2(output_data, LATEX_FILE):
    ...
    with open(LATEX_FILE, "a") as latex_file:
        ...
        latex_file.write("\\begin{longtable}{|c|c|c|c|c|c|c|c|c|} \n")
        latex_file.write("\\hline \n")
        latex_file.write("Voltage & \multirow{2}{*}{Meas.} & $V_c com$ & $V_c dif$ & $DH_m$ & $DH_{dep}$ & \multirow{2}{*}{Point} & $DH$ & Pressure \\\\ \n")
        ...
        for group in output_data:
            ...
            # Add a line above the voltage value
            latex_file.write("\\hline \n")
        ...
        # End of the table
        latex_file.write("\\hline \n\\end{longtable}\n")
        latex_file.write("This table contains values for the volume calculated from compression, but it also shows a volume from diffusion. It also contains permeability values, which are obtained for the maximum achieved pressure and for three additional points from this point to the end of the measurement, with the pressure at that point provided for comparison. \\ \n")

def export_LaTeX_3(output_data, LATEX_FILE):
    """Adds a table with average values for a given voltage to an existing LaTeX TXT file."""

    with open(LATEX_FILE, "a") as latex_file:
        # Start of the table
        latex_file.write("\\section*{Table 1: Average values for a given voltage}\n")
        latex_file.write("\\vspace{5mm}\n")
        latex_file.write("\\setlength{\LTleft}{1cm} \n")
        latex_file.write("\\begin{longtable}{|c|c|c|c|c|} \n")
        latex_file.write("\\hline \n")
        latex_file.write("Voltage & Max. Pressure & Eq. time & Max. eff. & Eff. time \\\ (V) & (bar) & (h) & (\%) & (h) \\\ \n")
        latex_file.write("\\hline \n")
        latex_file.write("\\endfirsthead \n")
        latex_file.write("\\hline \n")
        latex_file.write("Voltage & Max. Pressure & Eq. time & Max. eff. & Eff. time \\\ (V) & (bar) & (h) & (\%) & (h) \\\ \n")
        ...
        for group in output_data:
            ...
            if count > 0:  # If there are values for the given voltage
                # Calculate averages
                ...
                latex_file.write(f"\\hline\n")
                latex_file.write(f"{x_value} & {avg_pressure:.2f} & {avg_pressure_time:.2f} & {avg_work_eff:.1f} & {avg_work_eff_time:.2f} \\\\ \n")

        latex_file.write("\\hline \n")
        latex_file.write("\\end{longtable}\n")

        latex_file.write("\\vspace{5mm}\n")
        latex_file.write("\\setlength{\LTleft}{0cm} \n")
        latex_file.write("\\begin{longtable}{|c|c|c|c|c|c|} \n")
        latex_file.write("\\hline \n")
        latex_file.write("Voltage & Consumption & $V_c com$ & $V_c dif$ & DH & $DH_{dep}$ \\\ (V) & (kWh/kg) & (cm$^{3}$) & (cm$^{3}$) & (mol/Pa/m/s) & (mol/Pa/m/s) \\\ \n")
        latex_file.write("\\hline \n")
        latex_file.write("\\endfirsthead \n")
        latex_file.write("\\hline \n")
        latex_file.write("Voltage & Consumption & $V_c com$ & $V_c dif$ & DH & $DH_{dep}$ \\\ (V) & (kWh/kg) & (cm$^{3}$) & (cm$^{3}$) & (mol/Pa/m/s) & (mol/Pa/m/s) \\\ \n")
        ...
        for group in output_data:
            ...
            if count > 0:  # If there are values for the given voltage
                ...
                # Add average values to the table
                latex_file.write(f"\\hline\n")
                latex_file.write(f"{x_value} & {avg_EC:.2f} & {avg_vc_value:.2f} & {avg_vc_fit:.2f} & {avg_DH:.2e} & {avg_DH_depression:.2e}\\\\ \n")

        latex_file.write("\\hline \n")
        latex_file.write("\\end{longtable}\n")

        latex_file.write("This table contains average values for the given voltage obtained from all measurements for that voltage. It includes maximum pressure, time to reach equilibrium (pressure corresponding to 99 \% of the maximum pressure), maximum efficiency (with the time to reach it), and consumption at that time. At the end of the table are average values for the volume calculated from compression or diffusion and permeability.\\ \n")
