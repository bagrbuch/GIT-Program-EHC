...
#rom scipy.stats import linregress
...
def detect_linear_region_and_calculate_dpdt(pressure1, window_size=30, threshold=0.6):
    start_index = 0
    ...
    def modify_multiple_lines_in_file_with_time(file_name, line_numbers, first_time, second_time):
        ...
        for line_number in line_numbers:
            if line_number < 0 or line_number >= len(lines):
                print(f"Line {line_number + 1} does not exist in file.")
                continue
...
# permeability DH and back diffusion
    ...
    P_peak = pressure1[max_index] * 10**3 - 101325

    I_interval1 = A * current_density1[interval1]
    P_interval1 = pressure1[interval1] * 10**3 - 101325

    I_interval2 = A * current_density1[interval2]
    P_interval2 = pressure1[interval2] * 10**3 - 101325

    I_end = A * current_density1[-1]
    P_end = pressure1[-1] * 10**3 - 101325
    ...
    Peq = np.mean(pressure1[-100:])*10**3 - 101325   #[Pa]

    DH = Ieq * d
    DH = DH/(2*F*A*Peq)                       #[mol/Pa/m/s]

    back_diffusion = DH*A*np.array(pressure1*1000 - 101325)
...
#Volume
    ...
    print(f"\nRows {linear_end_index}, dP_dt: {dP_dt:.14f}")

    # dP_dt = (pressure1[20]-pressure1[1])/20
    # print(f"dP_dt: {dP_dt}")
    
    Vc = (A*np.mean(current_density1[1:2])*R*T)/(2*F*1000*dP_dt)
    VC = Vc*1000000
...
#Wh to compress H2 until 300kPa
    ...
    # Data
    rows = [
        ("Peak", DH_peak, (P_peak +101325)/ 100000),
        (" 1/3 ", DH_interval1, (P_interval1+101325) / 100000),
        (" 2/3 ", DH_interval2, (P_interval2+101325) / 100000),
        (" End", DH_end, (P_end+101325) / 100000),
    ]
...
# -------------------- Spotřeba energie vs tlak -------------------
            ...
            ax6.set_title('Energy consumption vs Pressure')
            plt.tight_layout()
