...
def find_and_trim_csv(file_path, ca_file_path, output_path=None,  chunk_size=2000, sub_chunk_size=500, expand_size=500, small_chunk_size=100, threshold=0.1, diff_threshold=0.015):
    ...
    try:
        ...
        if final_trimmed_rows:
            if output_path is None:
                # If output_path is not provided, generate it automatically in the same folder
                input_dir = os.path.dirname(os.path.abspath(file_path))
                output_path = os.path.join(input_dir, os.path.basename(file_path).replace(".csv", "_trimmed.csv"))
            ...
            time_difference = get_relative_time_difference(ca_file_path)
            found_row = find_row_by_time_difference(output_path, time_difference)
            split_file_by_row(output_path, found_row)
...
def split_file_by_row(input_file_path, row_number):
    ...
    try:
        ...
        compression_file_path = os.path.join(input_dir, 'compression.csv')
        diffusion_file_path = os.path.join(input_dir, 'diffusion.csv')

        with open(compression_file_path, mode='w', encoding='utf-8', newline='') as compression_file:
            writer = csv.writer(compression_file, delimiter=';')
            writer.writerow(header)
            writer.writerows(compression_rows)

        with open(diffusion_file_path, mode='w', encoding='utf-8', newline='') as diffusion_file:
